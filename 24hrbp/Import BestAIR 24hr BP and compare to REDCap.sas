...
*designed to run as part of "Run All.sas" - if running independently, uncomment %include "...create rand set.sas";
...
***************************************************************************************;
  ...
  *create formated table with some additional variables of statistical calculations such as mean, standard deviation, etc.;
  ...
  proc format;
    value dippingstatusf
      -1 = "-1: Reverse Dipper (Dipping% < 0)"
      0 = "0: Non-Dipper (0 =< Dipping% < 10)"
      1 = "1: Normal Dipper (Dipping% >= 10)"
    ;
  run;
  ...
  *merge pristine data and functions calculated in previous data steps;
  data bp24hr;
    ...
    *add dipping calculations;
    ...
    format sysdipstatus diadipstatus mapdipstatus dippingstatusf.;

    if sysdip = . then sysdipstatus = .;
    else if sysdip ge .10 then sysdipstatus = 1;
    else if sysdip ge 0 then sysdipstatus = 0;
    else sysdipstatus = -1;

    if diadip = . then diadipstatus = .;
    else if diadip ge .10 then diadipstatus = 1;
    else if diadip ge 0 then diadipstatus = 0;
    else diadipstatus = -1;

    if mapdip = . then mapdipstatus = .;
    else if mapdip ge .10 then mapdipstatus = 1;
    else if mapdip ge 0 then mapdipstatus = 0;
    else mapdipstatus = -1;
...
*****************************************************************************************;
  ...
  *rename variables from previous dataset to match variables from current dataset;
  data redcapbp;
    ...
    length timepoint 3;
    set redcap_rand (where = (elig_studyid ne . and monitorqc_studyvisit ne .));

      timepoint  = monitorqc_studyvisit;
      studyid = elig_studyid;

    keep monitorqc_studyid--monitor_24_hr_qc_complete timepoint studyid bpj_timebed bpj_timewake bplog_attachdate bplog_removaldate bplog_monitorbpr bplog_monitorbpl;
...
%include "&bestairpath\SAS\24hrbp\additional 24hrBP checks.sas";
...
*******************************************************************************************;
  ...
  *drop variables not stored in final dataset;
  data redcapbp_final;
    set redcapbp;
    drop bpj_timebed bpj_timewake bplog_attachdate bplog_removaldate bplog_monitorbpr bplog_monitorbpl;
  run;

  *merge the datasets from raw data files and manually entered QC forms;
  data mergebp;
    merge bestairbp24hr(in=a) redcapbp_final(in=b);
    by studyid timepoint;
  ...
  %include "&bestairpath\SAS\24hrbp\mergebp_labels.sas";
