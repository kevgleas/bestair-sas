...
data crfs2 (drop = exclude_from6 exclude_from12);
  merge crfs2 dropoutlist;
  by elig_studyid;

  if timepoint = 0 then output crfs2;
  else if timepoint = 6 then do;
    if exclude_from6 ne 1 then output crfs2;
    end;
  else if timepoint = 12 then do;
    if exclude_from12 ne 1 then output crfs2;
    end;
run;
