...
*import dropout list to exclude dropouts from expected data;
...
*restrict dataset to those from whom we expect data (exclude dropouts);
data questionnaires1 (drop = exclude_from6 exclude_from12);
  merge questionnaires1 dropoutlist;
  by elig_studyid;

  if timepoint = 0 then output questionnaires1;
  else if timepoint = 6 then do;
    if exclude_from6 ne 1 then output questionnaires1;
    end;
  else if timepoint = 12 then do;
    if exclude_from12 ne 1 then output questionnaires1;
    end;
run;
